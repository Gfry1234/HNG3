...
export default function Error({
  ...
  const handleTryAgain = async () => {
    ...
    // First check if we're online now
    if (!navigator.onLine) {
      // Still offline, just reload to try again
      window.location.reload();
      return;
    }
    
    // We're online, do a full page reload to fetch fresh data
    // This clears any stale state and triggers a fresh fetch
    window.location.reload();
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0a0e27] via-[#1a1f4e] to-[#2d1b69] p-4">
      <div className="glass-strong rounded-3xl p-8 max-w-md w-full text-center shadow-2xl animate-fade-in">
        ...
        <p className="text-white/60 text-sm mb-6 leading-relaxed">
          {isNetworkError
            ...
            : "The app encountered an error. Let's fetch fresh data."}
        </p>
